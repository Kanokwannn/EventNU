...
if (!empty($email)) {
    $sql = "
        SELECT sa.first_name, sa.studentaffairs_email, sa.personal_id, sa.position, 
               sa.phone, g.genderType, sa.last_name, sa.genderID
        FROM studentaffairs sa
        LEFT JOIN gender g ON sa.genderID = g.genderID
        WHERE sa.studentaffairs_email = ?
    ";
...
<body class="navbar-body">
            ...
            <div class="nav-right">
                ...
                <div class="user-menu" id="userMenu">
                    <div class="d-flex align-items-center px-3 py-2">
                        ...
                        <div>
                            <?php
                            if (isset($user['first_name']) && isset($user['studentaffairs_email'])) {
                                echo '<p class="m-0">' . htmlspecialchars($user['first_name']) . '<br><small>' . htmlspecialchars($user['studentaffairs_email']) . '</small></p>';
                            } else {
                    ...
                    <hr>
                    <a href="../logput.php" class="text-danger">ออกจากระบบ</a>
                </div>
            </div>
        </div>
    </nav>
    ...
    <div class="setting-setting-container">
        <div class="setting-setting-container-data">
            ...
            <div class="setting-setting-menu">
                <a href="eventAll.php" class="setting-setting-menu-item"><i class="bi bi-ticket-perforated"></i>
                    อีเว้นท์ทั้งหมด</a>
                <a href="allOrder.php" class="setting-setting-menu-item"><i class="bi bi-clock-history"></i>
                    คำสั่งซื้อทั้งหมด</a>
                <a href="addEvent.php" class="setting-setting-menu-item"><i class="bi bi-clipboard2-plus"></i>
                    เพิ่มอีเว้นท์</a>
                <a href="changeRole.php" class="setting-setting-menu-item"><i class="bi bi-person-plus"></i>
                    คำขอจัดอีเว้นท์</a>
                <a href="setting.php" class="setting-setting-menu-item active"><i class="bi bi-gear-fill"></i>
                    ข้อมูลส่วนตัว</a>
                <a href="../logout.php" class="setting-setting-menu-item setting-setting-logout"><i
        ...
        <div class="setting-setting-container-extra-info">
            ...
            <div class="setting-personal-info" id="personalInfo">
                ...
                <p>ชื่อ-นามสกุล: <?php echo htmlspecialchars($user['first_name']) ?></p>
                <p>รหัสประจำตัว: <?php echo htmlspecialchars($user['personal_id']) ?></p>
                <p>วันเกิด: <?php echo htmlspecialchars($user['birth_date']) ?></p>
                <p>เพศ: <?php echo htmlspecialchars($user['genderType']) ?></p>
                <p>ตำแหน่ง: <?php echo htmlspecialchars($user['position']) ?></p>
                <p>เบอร์โทรศัพท์มือถือ: <?php echo htmlspecialchars($user['phone']) ?></p>
            </div>
            ...
            <div class="setting-edit-form" id="editForm" style="display: none;">
                ...
                <form id="editProfileForm">
                    ...
                    <input type="text" id="phoneNumber" name="phoneNumber"
                        value="<?php echo htmlspecialchars($user['phone']) ?>" maxlength="12"
